...
- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    else if(self.selectedPhotos.count>8){
        return;
    }
    else{
        [self addPhotoSelectedView:[self.photosThumbnailLibrairy objectAtIndex:indexPath.row]
...
//    NSValue* keyboardFrameBegin1 = [keyboardInfo valueForKey:UIKeyboardFrameBeginUserInfoKey];
//    CGRect keyboardFrameBeginRect1 = [keyboardFrameBegin1 CGRectValue];
//    NSLog(@"%@%@",NSStringFromCGRect(keyboardFrameBeginRect),NSStringFromCGRect(keyboardFrameBeginRect1));
    
    self.state = true;
    [UIView animateWithDuration:0.5 animations:^{
        self.textView.frame = CGRectMake(self.textView.frame.origin.x, self.textView.frame.origin.y,self.textView.frame.size.width, ([UIScreen mainScreen].bounds.size.height - 64) -
                                         keyboardFrameBeginRect.size.height - 40 - self.selectedPhotosView.frame.size.height);
    }];
    self.buttonAddPhoto.frame = CGRectMake(self.buttonAddPhoto.frame.origin.x, [UIScreen mainScreen].bounds.size.height -
                                           keyboardFrameBeginRect.size.height - 30, self.buttonAddPhoto.frame.size.width,
                                           self.buttonAddPhoto.frame.size.height);
    
    self.numberLine.frame = CGRectMake(self.numberLine.frame.origin.x, [UIScreen mainScreen].bounds.size.height -
                                       keyboardFrameBeginRect.size.height - 30, self.numberLine.frame.size.width,
                                       self.numberLine.frame.size.height);
    
    self.photosCollection.frame = CGRectMake(0, [UIScreen mainScreen].bounds.size.height - keyboardFrameBeginRect.size.height,
                                             [UIScreen mainScreen].bounds.size.width, keyboardFrameBeginRect.size.height);
}
...
- (void) initPanelButton {
    ...
    imageButton.image = [UIImage imageNamed:@"photo"];
...
- (void) initPhotosCollection {
    ...
    layoutCollection.itemSize = CGSizeMake([UIScreen mainScreen].bounds.size.width / 4 - 2, [UIScreen mainScreen].bounds.size.width / 4 - 2);
...
- (void) initTextView {
    self.textView = [[UITextView alloc] initWithFrame:CGRectMake(5, self.navigationBar.frame.size.height + 5,
                                                                 [UIScreen mainScreen].bounds.size.width - 10, 0)];
...
- (void) initUI {
    ...
    UINavigationItem *item = [[UINavigationItem alloc] initWithTitle:@"分享"];
